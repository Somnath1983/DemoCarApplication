...
namespace PetApplication.Service
{
    public class GetPetServiceData : IGetPetServiceData
    {
        ...
        public async Task<IEnumerable<PetOwnerModel>> GetPetDataFromService()
        {
            try
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    {
                        petOwnerResult = await httpClient.GetStringAsync(new Uri(ConfigurationManager.AppSettings[Constant.ApiUrl]));
